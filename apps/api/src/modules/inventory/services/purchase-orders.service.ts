  ...
  async updateStatus(
    ...
    if (
      ...
      throw new ConflictException(
        `Cannot change status of a completed or cancelled Purchase Order (Current: ${po.status})`,
      );
    }
    ...
    if (
      ...
      throw new ConflictException(
        `Purchase Order must be in PENDING_APPROVAL status to be APPROVED (Current: ${po.status})`,
      );
    }
    ...
    if (
      ...
      throw new ConflictException(
        `Purchase Order must be APPROVED or ORDERED to be RECEIVED (Current: ${po.status})`,
      );
    }
