...
export class ForecastingController {
  ...
  @Get('sku/:id')
  @ApiOperation({ summary: 'Get 30, 60, and 90-day demand forecast for a specific SKU' })
  getSkuForecast(@Param('id') id: string, @Request() req: any) {
    return this.forecastingService.getSkuForecast(req.user.tenantId, id);
  }

  @Get('top-demand')
  @ApiOperation({ summary: 'Get top products with highest forecasted demand in next 30 days' })
  getTopDemand(@Request() req: any) {
    return this.forecastingService.getTopDemand(req.user.tenantId);
  }

  @Post('train')
  @ApiOperation({ summary: 'Trigger model retraining for all active products in the tenant' })
